...
namespace TechWorldAPI.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public OrderController(TechWorldDBContext context, IMapper mapper, ILogger<OrderController> logger)
        {
            _context = context;
            _mapper = mapper;
            _logger = logger;
        ...
        public async Task<ActionResult<OrderResponseDTO>> CreateOrder([FromBody] OrderCreateDTO orderRequest)
        {
            try
            {
                _logger.LogInformation($"📥 TechWorld: Creating order: {orderRequest.OrderNumber}");
                ...
                _logger.LogInformation($"✅ TechWorld: Order created successfully in database");
                ...
                _logger.LogInformation($"✅ TechWorld: Order and OrderItems tables updated successfully");
                ...
                var orderConfirmation = new
                {
                    ...
                    estimatedDeliveryDays = 1, // TechWorld delivery estimate
                    estimatedDeliveryDate = DateTime.UtcNow.AddDays(1).ToString("yyyy-MM-dd"),
                    distributorName = "TechWorld"
                };
                
                _logger.LogInformation($"✅ TechWorld: Order confirmation sent - Order ID: {orderDTO.Id}, Delivery: {DateTime.UtcNow.AddDays(1):yyyy-MM-dd}");
                
                return CreatedAtAction(nameof(GetOrder), new { id = orderDTO.Id }, orderConfirmation);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ TechWorld: Error creating order");
