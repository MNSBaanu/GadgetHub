...
namespace GadgetCentralAPI.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public OrderController(GadgetCentralDBContext context, IMapper mapper, ILogger<OrderController> logger)
        {
            _context = context;
            _mapper = mapper;
            _logger = logger;
        ...
        public async Task<ActionResult<OrderResponseDTO>> CreateOrder([FromBody] OrderCreateDTO orderRequest)
        {
            try
            {
                _logger.LogInformation($"📥 GadgetCentral: Creating order: {orderRequest.OrderNumber}");
                ...
                _logger.LogInformation($"✅ GadgetCentral: Order created successfully in database");
                ...
                _logger.LogInformation($"✅ GadgetCentral: Order and OrderItems tables updated successfully");
                ...
                var orderConfirmation = new
                {
                    ...
                    estimatedDeliveryDays = 3, // GadgetCentral delivery estimate
                    estimatedDeliveryDate = DateTime.UtcNow.AddDays(3).ToString("yyyy-MM-dd"),
                    distributorName = "GadgetCentral"
                };
                
                _logger.LogInformation($"✅ GadgetCentral: Order confirmation sent - Order ID: {orderDTO.Id}, Delivery: {DateTime.UtcNow.AddDays(3):yyyy-MM-dd}");
                
                return CreatedAtAction(nameof(GetOrder), new { id = orderDTO.Id }, orderConfirmation);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ GadgetCentral: Error creating order");
